...
const navigation = [
    ...
    { name: 'Item', href: '/item', current: true },
...
function sortCharactersByStartDate(item) {
  return item.slice().sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
}

function Item() {
  const [itemData, setItemData] = useState([]);
  const sortedCharacterData = sortCharactersByStartDate(itemData);
        ...
        <div className="container mx-auto max-w-3/4 border-1 bg-blue-600 p-4 rounded">
          <div className="mx-auto max-w-screen-lg py-6 sm:px-6 lg:px-8 ">
            <div className="grid grid-cols-4 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-8 xl:grid-cols-8 gap-2 ">
              {sortedCharacterData.map((item, index) => (
                <div
                  key={index}
